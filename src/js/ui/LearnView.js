...
function buildMusicXMLFor(step, opts) {
  ...
  const measuresXml = measures.map((m, i) => {
    const raw = symbols && symbols[i] ? symbols[i] : '';
    const sym = transposeSemis ? transposeChordSymbol(raw, transposeSemis) : raw;
    const harmonyXml = sym ? harmonyTagFor(sym) : '';
    return `
    <measure number="${i + 1}">
      ${i === 0 ? `<attributes>
...
function harmonyTagFor(symbol) {
  ...
  return `<harmony><root><root-step>${stepLetter}</root-step>${alterTag}</root><kind text="${escapeText(symbol)}">${kind}</kind>${bassXml}</harmony>`;
}
